...
- `CellValue::Continuous` の場合、`max` は `f64::max` に対応する。
- 勾配計算が必要になった場合、`max` を smooth max（LogSumExp 等）に置き換える。
- 初期実装では `Binary` のみをサポートし、`Continuous` はデータ構造として定義だけしておく。
