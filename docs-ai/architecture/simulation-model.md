...
`SimulatorSimple` はモジュール付き回路に対し、親シミュレータ内で子シミュレータを再帰的に保持する。

主要フィールド:

- `sub_simulators: Vec<SimulatorSimple>`: 各モジュールに対応する子シミュレータ
- `module_output_cells: HashSet<usize>`: モジュール出力セル（通常の wire 合成をスキップ）
- `module_triggers: HashMap<usize, usize>`: 親セルインデックス → モジュールインデックス

### 実行タイミング

親 `step()` 中に次を行う:

1. 現在セルが `module_triggers` に存在するかを確認
2. 存在する場合、`evaluate_module(module_index)` を呼び出し
3. `evaluate_module` は親入力セル値を子 `sub_input` に注入し、子を 1 tick 実行して `sub_output` を親出力セルに反映
4. 出力セルは `module_output_cells` 判定により通常のワイヤ合成を行わない

この設計により、モジュールをブラックボックスとして扱いながら、親回路の tick 粒度と整合した評価が可能になる。

### 可視性

- `TickOutput` / `get_state()` が返すのは親回路セルのみ
- サブ回路内部セルは外部 API には露出しない
- モジュール出力セルは親回路セルとして取得可能
