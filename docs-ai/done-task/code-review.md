...
**対象:** `src/` 以下の全ソースコード  
**日付:** 2026-03-23  
**ステータス:** #2,#3,#4 対応済み / #1,#5,#6 個別タスクに分割済み

---

## 1. ~~clap 依存が library crate 全体に含まれている~~ (分割済み → [separate-clap-dependency.md](../task/separate-clap-dependency.md))
...
## 2. ~~SRP 違反: `io/json.rs` にシミュレーション実行ロジックが含まれる~~ (対応済み)
...
## 3. ~~モジュール名と内容の不一致: `cell.rs` に `Pos` のみ~~ (対応済み)

**重要度:** medium  
**対象:** [src/circuit/pos.rs](../../src/circuit/pos.rs)
...
## 5. ~~`simulate_to_output_json` 内の冗長なソート~~ (分割済み → [remove-redundant-sort.md](../task/remove-redundant-sort.md))
...
## 6. ~~エラー型に `String` を使用~~ (分割済み → [structured-error-types.md](../task/structured-error-types.md))
