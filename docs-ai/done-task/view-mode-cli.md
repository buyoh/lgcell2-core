...
- [interactive-cli.md](../task/interactive-cli.md): インタラクティブモード（テキストコマンドベース）。`--view` とは排他
...
- `src/platform/console.rs` を追加し、`Console` トレイトと `CrosstermConsole` を実装
- `src/view/renderer.rs` を追加し、レンダリングロジックを端末依存コードから分離
- `src/view/renderer_tests.rs` を追加し、レンダリングの Unit-Fake test を実装
- `src/bin/lgcell2/view.rs` を追加し、イベントループ（`q` / `Space` / 矢印）と tick 自動進行を実装
- `src/bin/lgcell2/main.rs` を更新し、`--view` モード分岐と `--ticks` 排他チェックを追加
- `Cargo.toml` を更新し、`crossterm` を `cli` feature 配下の optional dependency として追加
- `src/lib.rs` を更新し、`platform` / `view` モジュールを公開

### テスト実行結果

- 実行コマンド: `cargo test --features cli`
- 結果: 全テスト成功（新規失敗なし）
