...
┌─ WireSimulator ─────────────────────────────┐
│  circuit: Circuit                            │
│  wire_state: WireSimState                    │  ← 遅延ワイヤ + 入力なしセルの状態
│  cell_values: Vec<bool>                      │  ← tick 中の計算用
│  cell_pos_to_index: HashMap<Pos, usize>      │  ← 座標→インデックス（事前計算）
│  tick: u64                                   │
│  cell_index: usize                           │
...
│  slots: Vec<DelayedSlot>                     │  ← スロット種別
│  wire_to_slot: HashMap<usize, usize>         │  ← ワイヤ→スロット索引
│  cell_to_slot: HashMap<usize, usize>         │  ← 入力なしセル→スロット索引
└──────────────────────────────────────────────┘
```
...
構築時に遅延ワイヤのインデックスを `WireSimState` の `wire_to_slot` で管理する。

#### tick 処理フロー（新モデル）

1. Generator 等の入力を `cell_values` に適用
2. 各セルを辞書順に処理:
   - 入力ワイヤなし（Generator 対象）: `apply_inputs()` で設定済みのため何もしない
   - 入力ワイヤなし（Generator 対象外）: `delayed_values` から前 tick の値を復元し `cell_values` に書き込む
...
1. [x] **`WireSimState` の実装**: 遅延ワイヤベースの状態管理（[wire-sim-state.md](wire-sim-state.md)）
2. [x] **`WireSimulator` の実装**: ワイヤ状態モデルのシミュレーションエンジン（[wire-simulator.md](wire-simulator.md)）
3. [x] **出力形式の実装**: AllCell / ViewPort 形式の対応（[output-format.md](output-format.md)）
4. [x] **利用側の移行**: `WasmSimulator`, `io::json`, CLI を `WireSimulator` へ直接切り替え（[trait-design.md](trait-design.md)）
5. [x] **旧実装の削除**: `Simulator`, `SimState`, `StateMut` を削除
6. [x] **テスト**: テストマニフェスト・エンジンテストで `WireSimulator` を検証

## 実装進捗（2026-03-29）

- `src/simulation` を `WireSimulator` / `WireSimState` 構成へ移行
- `TickSnapshot` を `TickOutput`（`HashMap<Pos, bool>`）へ移行
- `OutputFormat`（`AllCell` / `ViewPort`）と `Rect` を実装
- `io::json`, `wasm_api::WasmSimulator`, `view::renderer`, `bin/lgcell2/view.rs`, `tests/test_helpers.rs` を新 API に移行
- 旧 `src/simulation/state.rs` を削除
